        ...
        <section className="bg-white border rounded p-5">
          {existingSample? (
            ...
            <div>
              ...
              <SampleRequestButton offers={offers||[]} roomId={room?.id||""} buyerEmail={participants?.[0]?.email||""} roomName={room.name} />
            </div>
          )}
        </section>
            ...
            {offers?.map((offer:any)=>(
              <div key={offer.id} className="px-5 py-4">
                ...
                <div className="mt-3 flex gap-2">
                  ...
                  <PurchaseOrderButton roomId={room?.id||""} buyerEmail={participants?.[0]?.email||""} offerId={offer.id} price={offer.price_per_kg ?? 0} purchaseOrder={purchaseOrders?.find((p:any)=>p.offer_id===offer.id)} offer={offer} />
                </div>
              </div>
            ))}
          </div>
