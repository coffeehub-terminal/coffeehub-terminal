...
export default function Home() {
  ...
  const loadBadges = async()=>{
    const {count: sCount} = await supabase.from("Samples").select("*",{count:"exact", head:true}).eq("status","Requested");
    if(sCount) setPendingSamples(sCount);
    const {count: poCount} = await supabase.from("PurchaseOrders").select("*",{count:"exact", head:true}).in("status",["Pending","Requested"]);
    if(poCount) setPendingPOs(poCount);
    const {count: cCount} = await supabase.from("Contracts").select("*",{count:"exact", head:true}).eq("status","Draft");
    if(cCount) setPendingContracts(cCount);
    const {count: lCount} = await supabase.from("Logistics").select("*",{count:"exact", head:true}).eq("status","Booking Requested");
    if(lCount) setPendingLogistics(lCount);
  };
      ...
      <div className="sticky top-0 z-20 bg-white border-b border-[#EAE6E1]">
        <div className="max-w-[1280px] mx-auto px-6 h-14 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <Link href="/purchase-orders" className="h-8 px-3 rounded-full border border-[#E8E8E8] text-[12px] flex items-center hover:bg-[#111] hover:text-white transition">POs {pendingPOs>0 && <span className="ml-1 bg-amber-500 text-white px-1.5 py-0.5 rounded-full text-[10px]">{pendingPOs}</span>}</Link>
            <Link href="/contracts" className="h-8 px-3 rounded-full border border-[#E8E8E8] text-[12px] flex items-center hover:bg-[#111] hover:text-white transition">Contracts {pendingContracts>0 && <span className="ml-1 bg-amber-500 text-white px-1.5 py-0.5 rounded-full text-[10px]">{pendingContracts}</span>}</Link>
            <Link href="/logistics" className="h-8 px-3 rounded-full border border-[#E8E8E8] text-[12px] flex items-center hover:bg-[#111] hover:text-white transition">Logistics {pendingLogistics>0 && <span className="ml-1 bg-amber-500 text-white px-1.5 py-0.5 rounded-full text-[10px]">{pendingLogistics}</span>}</Link>
            <Link href="/samples" className="h-8 px-3 rounded-full border border-[#E8E8E8] text-[12px] flex items-center hover:bg-[#111] hover:text-white transition">Samples {pendingSamples>0 && <span className="ml-1 bg-red-500 text-white px-1.5 py-0.5 rounded-full text-[10px]">{pendingSamples}</span>}</Link>
          </div>
        </div>
      </div>
