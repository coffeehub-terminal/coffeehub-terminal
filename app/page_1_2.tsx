...
export default function Home() {
  ...
  const createRoom = async()=>{
    ...
    const selectedOffers = offers.filter((o:any)=>selectedOfferIds.includes(o.id));
    try { if(participantEmail){ const res=await fetch("/api/send-room",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:participantEmail,roomName:newRoomName,buyerLink,offers:selectedOffers})}); const j=await res.json(); if(!res.ok) alert(`Room created but email failed: ${j.error}\n${buyerLink}`); else alert(`Room created + email sent to ${participantEmail}!\n${buyerLink}`);} else { alert(`Room created!\n${buyerLink}`);} } catch(e:any){ alert(`Room created email error: ${e.message}\n${buyerLink}`);} 
    setCopied(buyerLink);
